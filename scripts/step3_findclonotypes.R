...
dir.sample = snakemake@params[['dir_sample']]
output = snakemake@output[['seurat_out']]
...
if(length(name.ID) > 1){
  seurat.list = sapply(seq_along(name.ID), function(x) readRDS(paste0(dir.sample[x], "/signac/SeuratObject_", name.ID[x], ".s2.rds")))
  message("Merging samples from the same condition")
  seurat = merge(x = seurat.list[[1]], y = seurat.list[names(seurat.list) != name.ID[1]])
} else {
  seurat = readRDS(paste0(dir.sample, "/signac/SeuratObject_", name.ID, ".s2.rds"))
}
